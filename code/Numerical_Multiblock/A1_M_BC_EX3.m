...
for j = 1:size(start,1)   
    ...
    start_F_test_rslt = [];
    start_F_eval_rslt = [];
    start_total_iter_rslt = [];
    start_outer_iter_rslt = [];
    start_N_rslt = [];
    start_band_rslt = [];
    ...
    like0 = @(x,theta0,theta1,theta2,noise0,mse) -(1/2)*log(2*pi) -(1/2)*log(mse) -(1/(2*mse))*(fc0(x,theta0,theta1,theta2) - fp0(x,Ttheta0,Ttheta1,Ttheta2,noise0)).^2;
    Hlike0 = @(x,theta0,theta1,theta2,noise0,mse) -(1/length(x))*sum((-like0(x,theta0+2*dt,theta1,theta2,noise0,mse) + 16*like0(x,theta0+dt,theta1,theta2,noise0,mse) - 30*like0(x,theta0,theta1,theta2,noise0,mse) + 16*like0(x,theta0-dt,theta1,theta2,noise0,mse) - like0(x,theta0-2*dt,theta1,theta2,noise0,mse))/(12*dt^2));   
    
    like1 = @(x,theta0,theta1,theta2,noise1,mse) -(1/2)*log(2*pi) -(1/2)*log(mse) -(1/(2*mse))*(fc1(x,theta0,theta1,theta2) - fp1(x,Ttheta0,Ttheta1,Ttheta2,noise1)).^2;
    Hlike1 = @(x,theta0,theta1,theta2,noise1,mse) -(1/length(x))*sum((-like1(x,theta0,theta1+2*dt,theta2,noise1,mse) + 16*like1(x,theta0,theta1+dt,theta2,noise1,mse) - 30*like1(x,theta0,theta1,theta2,noise1,mse) + 16*like1(x,theta0,theta1-dt,theta2,noise1,mse) - like1(x,theta0,theta1-2*dt,theta2,noise1,mse))/(12*dt^2));
    
    like2 = @(x,theta0,theta1,theta2,noise2,mse) -(1/2)*log(2*pi) -(1/2)*log(mse) -(1/(2*mse))*(fc2(x,theta0,theta1,theta2) - fp2(x,Ttheta0,Ttheta1,Ttheta2,noise2)).^2;
    Hlike2 = @(x,theta0,theta1,theta2,noise2,mse) -(1/length(x))*sum((-like2(x,theta0,theta1,theta2+2*dt,noise2,mse) + 16*like2(x,theta0,theta1,theta2+dt,noise2,mse) - 30*like2(x,theta0,theta1,theta2,noise2,mse) + 16*like2(x,theta0,theta1,theta2-dt,noise2,mse) - like2(x,theta0,theta1,theta2-2*dt,noise2,mse))/(12*dt^2));
...
%             eta2 = abs(new_F2c-old_F2c)/abs(old_F2c);
    ...
    temp_band = [1.96*sqrt(Hlike0(x0,theta0,theta1,theta2,noise0,Fta)^(-1))/sqrt(length(x0)) ...
                 1.96*sqrt(Hlike1(x1,theta0,theta1,theta2,noise1,Fta)^(-1))/sqrt(length(x1)) ...
                 1.96*sqrt(Hlike2(x2,theta0,theta1,theta2,noise2,Fta)^(-1))/sqrt(length(x2))]
    start_theta_rslt = [start_theta_rslt; tempT];
    start_F_rslt = [start_F_rslt; tempF];
    start_F_test_rslt = [start_F_test_rslt; tempF_test];
    start_F_eval_rslt = [start_F_eval_rslt; tempF_eval];
    start_total_iter_rslt = [start_total_iter_rslt; total_iter];
    start_outer_iter_rslt = [start_outer_iter_rslt; k];
    ...
    F_eval_mean_store = [F_eval_mean_store; mean(start_F_eval_rslt,1)];
    F_eval_std_store = [F_eval_std_store; std(start_F_eval_rslt,1)];
    total_iter_mean_store = [total_iter_mean_store; mean(start_total_iter_rslt,1)];
    total_iter_std_store = [total_iter_std_store; std(start_total_iter_rslt,1)];
    outer_iter_mean_store = [outer_iter_mean_store; mean(start_outer_iter_rslt,1)];
    outer_iter_std_store = [outer_iter_mean_store; std(start_outer_iter_rslt,1)];
...
save(['/results/EX' num2str(case_no) '_M_BC.mat'])
