...
eta <- function(x,theta,B,D) {
  e1 = (1/5)*(theta[1] - 1)*sqrt(x^2-x+1)*( x < max_x/(2*D+1) )
  e2 = (theta[1] - 1)*(theta[2] - 2)*sqrt(x^2-x+1)*( (x >= max_x/(2*D+1)) & (x < 2*max_x/(2*D+1)) ) 
  e3 = (1/5)*(theta[2] - 2)*sqrt(x^2-x+1)*( (x >= 2*max_x/(2*D+1)) & (x < (2*D-1)*max_x/(2*D+1)) )
  e4 = (theta[2] - 2)*(theta[3] - 3)*sqrt(x^2-x+1)*( (x >= (2*D-1)*max_x/(2*D+1)) & (x < 2*D*max_x/(2*D+1)) ) 
  e5 = (1/5)*(theta[3] - 3)*sqrt(x^2-x+1)*( x >= 2*D*max_x/(2*D+1) )
...
Hlike = function(x,theta,B,D,noise,mse) {
  H = numeric(0)
  for (i in c(1:B)) {
    H[i] = -(1/length(x))*sum((-like(x,theta+2*h*tabulate(i,nbins=B),B,D,noise,mse) + 16*like(x,theta+h*tabulate(i,nbins=B),B,D,noise,mse) -
  ...
  for (iTry in c(1:nTry)) {
    ...
    while (TRUE) {
      ...
      for (t in c(1:B)) {
        ...
        g = gradB(x_tr,theta,t,B,D,noise_tr)
        while ( funcB(x_tr,theta-alpha*g*tabulate(t,nbins=B),t,B,D,noise_tr) > funcB(x_tr,theta,t,B,D,noise_tr) - c1*alpha*norm(g,type="2")^2 ) {
          alpha = alpha*rho
        }
        # GD update 
      ...
      for (t in c(1:B)) {
        x_tr = x_train_list[[t]]
        noise_tr = noise_train_list[[t]]
        band_length[t] = 1.96*sqrt(HlikeB(x_tr,theta,t,B,D,noise_tr,mse)^(-1))/sqrt(length(x_tr)) 
      }
      
      if (k!=1) {
        xi = 0
        for (t in c(1:B)){
          xi = max(xi, norm(gradB(x_train_list[[t]],theta,t,B,D,noise_train[[t]]),"2") )
        }
