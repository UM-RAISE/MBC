...
%     gF2 = @(x,theta0,theta2) (F2(x,theta0,theta2+dt,noise2) - F2(x,theta0,theta2-dt,noise2))/(2*dt);      
    ...
    like0_0 = @(x,theta0,theta1,theta2,noiset,mse) (like0(x,theta0+dt,theta1,theta2,noiset,mse)-like0(x,theta0-dt,theta1,theta2,noiset,mse))/(2*dt);
    like0_1 = @(x,theta0,theta1,theta2,noiset,mse) (like0(x,theta0,theta1+dt,theta2,noiset,mse)-like0(x,theta0,theta1-dt,theta2,noiset,mse))/(2*dt);
    like0_2 = @(x,theta0,theta1,theta2,noiset,mse) (like0(x,theta0,theta1,theta2+dt,noiset,mse)-like0(x,theta0,theta1,theta2-dt,noiset,mse))/(2*dt);
    ...
    while 1       
        ...
        [tau0,f_eval_F] = back_without(theta,x_init,noise_init,case_no,f_eval_F);
...
%             delta = abs(new_F0-old_F0)/abs(old_F0);
    ...
    Imat = [I00(x_test_init,theta(1),theta(2),theta(3),noise_init,Fta) I01(x_test_init,theta(1),theta(2),theta(3),noise_init,Fta) I02(x_test_init,theta(1),theta(2),theta(3),noise_init,Fta);
         I10(x_test_init,theta(1),theta(2),theta(3),noise_init,Fta) I11(x_test_init,theta(1),theta(2),theta(3),noise_init,Fta) I12(x_test_init,theta(1),theta(2),theta(3),noise_init,Fta);
         I20(x_test_init,theta(1),theta(2),theta(3),noise_init,Fta) I21(x_test_init,theta(1),theta(2),theta(3),noise_init,Fta) I22(x_test_init,theta(1),theta(2),theta(3),noise_init,Fta)];
    invI = inv(Imat); 
    temp_band = [1.96*sqrt(invI(1,1))/sqrt(length(x_test_init)) ...
                 1.96*sqrt(invI(2,2))/sqrt(length(x_test_init)) ...
                 1.96*sqrt(invI(3,3))/sqrt(length(x_test_init))] 
...
save(['/results/EX' num2str(case_no) '_S_BC.mat'])
